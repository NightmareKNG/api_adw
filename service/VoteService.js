...
exports.getVoteAllSortedByPointDescending = function() {
  return new Promise(async function (resolve, reject) {
    try {
      var Vote = getCollection('vote');
      var data = await Vote.find().sort({ point: -1 }); // เรียงจากมากไปน้อยตามคอลัมน์ point
      console.log(data);
      if (data.length > 0) {
        resolve({ code: 200, data: data });
      } else {
...
exports.getVoteByUser = function(user_id) {
  return new Promise(async function (resolve, reject) {
    try {
      var vote = getCollection('vote');
      var data = await vote.find({ 'img.user.user_id': user_id });
      console.log(data);
      if (!data.empty) {
        resolve({ code: 200, data: data });
      } else {
